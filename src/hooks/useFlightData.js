...
const useFlightData = () => {
  ...
  const onLoadPrevMore = () => {
    ...
    setLoading(true);
    setFlights([...skeletonList, ...data]);
    const paramsObj = Object.fromEntries([...searchParams]);
    delete paramsObj.scheduleDate;
    fetchFlight({
      ...paramsObj,
      fromDateTime: undefined,
      toDateTime: data[0]?.scheduleDateTime
        ? moment(data[0]?.scheduleDateTime)
            .subtract(1, "m")
            .utcOffset("+01:00")
      ...
      .then(async (res) => {
        ...
        const newData2 = [
          ...response.sort(
            (a, b) => moment(a.scheduleDateTime) - moment(b.scheduleDateTime)
          ),
          ...data,
        ];
  ...
  const getFlight = (filter) => {
    ...
    fetchFlight({
      ...paramsObj,
      fromDateTime: paramsObj?.scheduleDate
        ? undefined
        : moment().format("yyyy-MM-DDTHH:mm:ss"),
      sort: "+scheduleDateTime",
      searchDateTimeField: paramsObj?.scheduleDate
        ? undefined
        : "scheduleDateTime",
      flightDirection: paramsObj?.flightDirection ?? "D",
    })
...
const mergeAirLines = async (items) => {
  return Promise.all(
    items.map(async (flight) => {
      const withAirlines = await fetchAirLine(
        flight?.prefixICAO || flight?.prefixIATA
      );
      const withAirCraft = await fetchAirCraft({ ...flight.aircraftType });
      const withDestination = await fetchDestination(
        flight.route?.destinations[0]
      );
      const flightStatus = flight.publicFlightState.flightStates
        .map((status) => flightStatusType?.[status])
        .join(",");
      const baggageStatus = flight.baggageClaim?.belts
        ?.map((status) => flightStatusType?.[status])
        .join(",");
      return {
        ...flight,
        airlinePublicName: withAirlines.data?.publicName || "-",
        airCraftName:
          withAirCraft.data.aircraftTypes?.[0]?.shortDescription || "-",
        destinationName: withDestination.data?.publicName?.english || "-",
